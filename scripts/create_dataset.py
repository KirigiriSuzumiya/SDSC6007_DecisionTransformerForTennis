...
def generate_dataset(env_id, num_episodes):
    ...
    model_path = "/root/SDSC6007_DecisionTransformerForTennis/external_resource/models--cleanrl--Tennis-v5-cleanba_ppo_envpool_impala_atari_wrapper-seed1/snapshots/3f60d80a32047766ad6c46a87ffe5f0e3da79ef0/cleanba_ppo_envpool_impala_atari_wrapper.cleanrl_model"
    ...
    dataset = data_collector.create_dataset(
        dataset_id=f"atari/{env_id[:-len('-v5')].lower()}/expert-v11",
        author="Yifan BU",
        author_email="boyifan1@126.com",
        algorithm_name="CleanBA PPO Impala",
        description=f"CleanBA PPO Impala expert dataset collected using Farama's Minari library locally. episodes:{num_episodes}",
...
for env_id in ENV_LIST:
    # if f"atari/{env_id[len('ALE/'):-len('-v5')].lower()}/expert-v100" in minari.list_remote_datasets(prefix="atari"):
