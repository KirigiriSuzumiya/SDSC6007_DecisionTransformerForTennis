...
from .framestack import frame_stack
...
class MobileNetDTPreprocessor():
    ...
    def __call__(self, observations, rewards, actions, dones):
        ...
        if self.stack_frame:
            stacked_s = frame_stack(obs_img, stack_size=self.stack_frame)
            s.append(np.array(stacked_s).reshape(1, -1, 4, obs_img[0].shape[1], obs_img[0].shape[2])) # 4 channel on frames, gray scale
        else:
        ...
        if self.mode == "train":
            ...
            rtg.append(np.ones((1, s[-1].shape[1], 1)))
        ...
        tlen = s[-1].shape[1]
        if self.stack_frame:
            s[-1] = np.concatenate([np.zeros((1, self.max_len - tlen, 4, s[-1].shape[3], s[-1].shape[4])), s[-1]], axis=1)
        else:
